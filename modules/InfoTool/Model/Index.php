...
class Index
{
    ...
    public static function injectToolbar(\Smarty $oView)
    {
        ...
        INJECT:
        {
            ...
            (!empty($sMarkup)) ? $oDom->loadHTML($sMarkup) : false;
            libxml_clear_errors();
    ...
    protected function collectInfo(\Smarty $oView)
    {
        ...
        $aToolbar['sTemplateContent'] = (file_exists($aToolbar['sTemplate'])) ? file_get_contents($aToolbar['sTemplate']) : '';

        ob_start();
        $sTemplate = (file_exists($oView->sTemplate)) ? file_get_contents($oView->sTemplate, true) : '';
